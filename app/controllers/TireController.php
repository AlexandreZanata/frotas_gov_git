...
class TireController
{
    ...
    {
        // Garante que o usuário tem permissão para ver a página
        if ($this->currentUser['role_id'] > 2) {
             show_error_page('Acesso Negado', 'Você não tem permissão para acessar o estoque.', 403);
        }

        $secretariatId = ($this->currentUser['role_id'] == 1) ? null : $this->currentUser['secretariat_id'];
        $whereClause = $secretariatId ? "WHERE t.secretariat_id = :secretariat_id" : "";
        $params = $secretariatId ? [':secretariat_id' => $secretariatId] : [];

        // Adiciona JOIN para buscar o nome da secretaria
        $stmt = $this->conn->prepare(
            "SELECT t.*, s.name as secretariat_name 
             FROM tires t
             LEFT JOIN secretariats s ON t.secretariat_id = s.id
             $whereClause ORDER BY t.created_at DESC"
        );
        $stmt->execute($params);
        $tiresInStock = $stmt->fetchAll();

        // Busca secretarias para o formulário (apenas para Admin Geral)
        $secretariats = [];
    ...
    {
        if ($this->currentUser['role_id'] > 2) {
            show_error_page('Acesso Negado', 'Você não tem permissão para esta ação.', 403);
        }
        ...
        if (empty($dot) || empty($brand) || empty($model)) {
            $_SESSION['error_message'] = "Os campos DOT, Marca e Modelo são obrigatórios.";
        ...
        if ($this->currentUser['role_id'] == 1) { // Admin Geral pode escolher
            $secretariat_id = filter_input(INPUT_POST, 'secretariat_id', FILTER_VALIDATE_INT);
            if (!$secretariat_id) {
                 $_SESSION['error_message'] = "Admin Geral deve selecionar uma secretaria.";
                 header('Location: ' . BASE_URL . '/tires/stock');
                 exit();
            }
        }
        ...
        try {
            ...
            $stmt_check = $this->conn->prepare("SELECT id FROM tires WHERE dot = ?");
            $stmt_check->execute([$dot]);
            if ($stmt_check->fetch()) {
                throw new Exception("O código DOT informado já está cadastrado no sistema.");
            }

            $stmt = $this->conn->prepare(
                "INSERT INTO tires (dot, brand, model, purchase_date, secretariat_id, status) 
                 VALUES (?, ?, ?, ?, ?, 'in_stock')"
            );
            $stmt->execute([$dot, $brand, $model, $purchase_date, $secretariat_id]);
            $lastId = $this->conn->lastInsertId();

            $this->auditLog->log($this->currentUser['id'], 'create_tire_stock', 'tires', $lastId, null, $_POST);
            $this->conn->commit();
            
            $_SESSION['success_message'] = "Pneu adicionado ao estoque com sucesso!";

        } catch (Exception $e) {
            if ($this->conn->inTransaction()) $this->conn->rollBack();
            $_SESSION['error_message'] = "Erro ao adicionar pneu: " . $e->getMessage();
        }
    ...
    public function settings()
    {
        ...
        // Busca categorias com suas regras de vida útil e layouts associados
        ...
        // Busca todas as categorias para o formulário de seleção
    ...
    /**
     * AJAX: Busca os detalhes (KM e Dias) de uma regra de vida útil.
     */
    ...
    /**
     * AJAX: Busca todos os layouts de pneus customizados.
     */
...
// Dentro de app/controllers/TireController.php

    /**
     * AJAX: Salva (cria ou atualiza) um layout de pneu.
     * VERSÃO CORRIGIDA PARA FORMULÁRIO SIMPLES.
     */
    public function ajax_store_layout()
    {
        ...
        // Espera uma string 'positions' e a converte para o JSON correto.
        $positions_string = $data['positions'] ?? '';
        $positions_array = array_filter(array_map('trim', explode(',', $positions_string)));
        
        if (empty($name) || empty($layout_key) || empty($positions_array)) {
            http_response_code(400);
            echo json_encode(['success' => false, 'message' => 'Nome, Chave e Posições dos pneus são obrigatórios.']);
            return;
        }
        
        // Cria o JSON de configuração que será salvo no banco
        $config_json = json_encode(['positions' => $positions_array]);
        ...
        try {
            ...
            $this->auditLog->log($_SESSION['user_id'], $action, 'tire_layouts', $recordId, null, ['name' => $name, 'layout_key' => $layout_key, 'positions' => $positions_string]);
            $this->conn->commit();
    ...
    /**
     * AJAX: Associa um layout_key a uma categoria de veículo.
     */
    ...
    public function ajax_get_vehicle_layout()
    {
        ...
        if (!$layoutInfo || empty($layoutInfo['config_json'])) {
            ...
            if (!$layoutInfo) {
                // Fallback extremo: se nem o layout padrão existir, cria um em memória
                $layoutInfo = [
                    'layout_key' => 'CAR_2X2',
                    'config_json' => json_encode(['positions' => ['front_left', 'front_right', 'rear_left', 'rear_right']])
                ];
            }
        }
        ...
        echo json_encode([
            'success' => true, 
            'layoutKey' => $layoutInfo['layout_key'], 
            'layoutConfig' => json_decode($layoutInfo['config_json']),
            'tires' => $tires
        ]);
    }
    /**
     * AJAX: Busca pneus disponíveis no estoque para instalação.
     */
    public function ajax_get_stock_tires()
    {
        header('Content-Type: application/json');
        $secretariatId = $_SESSION['user_secretariat_id'] ?? null;
        if ($_SESSION['user_role_id'] != 1) { // Se não for admin, filtra por secretaria
            $stmt = $this->conn->prepare("SELECT id, dot, brand, model FROM tires WHERE status = 'in_stock' AND secretariat_id = ? ORDER BY brand, model");
            $stmt->execute([$secretariatId]);
        } else {
            $stmt = $this->conn->query("SELECT id, dot, brand, model FROM tires WHERE status = 'in_stock' ORDER BY brand, model");
        }
        $tires = $stmt->fetchAll(PDO::FETCH_ASSOC);
        echo json_encode(['success' => true, 'data' => $tires]);
    }

    /**
     * AJAX: Processa ações de manutenção (rodízio, instalação, etc.).
     */
    public function ajax_perform_action()
    {
        ...
        try {
            $this->conn->beginTransaction();
            if ($action === 'rotate_internal') {
                $positions = $_POST['tires'] ?? [];
                if (count($positions) !== 2) throw new Exception("Selecione exatamente 2 pneus para o rodízio.");
                ...
                $this->conn->prepare("UPDATE vehicle_tires SET position = ? WHERE vehicle_id = ? AND tire_id = ?")->execute([$positions[1], $vehicleId, $tire1_id]);
                $this->conn->prepare("UPDATE vehicle_tires SET position = ? WHERE vehicle_id = ? AND tire_id = ?")->execute([$positions[0], $vehicleId, $tire2_id]);
            ...
            } elseif ($action === 'install_tire') {
                $position = filter_input(INPUT_POST, 'position', FILTER_SANITIZE_STRING);
                $tireId = filter_input(INPUT_POST, 'tire_id', FILTER_VALIDATE_INT);
                if (!$position || !$tireId) throw new Exception("Posição ou pneu inválido para instalação.");
                // Insere o novo pneu na posição
                $stmt_insert = $this->conn->prepare("INSERT INTO vehicle_tires (vehicle_id, tire_id, position) VALUES (?, ?, ?)");
                $stmt_insert->execute([$vehicleId, $tireId, $position]);
                // Atualiza o status do pneu no estoque
                $stmt_update = $this->conn->prepare("UPDATE tires SET status = 'in_use' WHERE id = ?");
                $stmt_update->execute([$tireId]);
                $description = "Pneu ID $tireId instalado no veículo ID $vehicleId na posição $position.";
                $this->logTireEvent($tireId, $userId, 'installation', $description);
            }
             else {
